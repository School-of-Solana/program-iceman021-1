...
**Deployed Frontend URL:** [LINK](https://sol-journal-two.vercel.app/ )
...
- **Update Entry**: Users can modify the message content of their existing entries.
- **Delete Entry**: Users can close the entry account and refund the rent (SOL) back to their wallet.
- **View Entries**: Automatic fetching and display of all on-chain entries owned by the connected wallet
...
Purpose: Ensures that every "Title" is unique per user. If a user tries to create an entry with a title they already used, the PDA collision detection will fail the transaction, enforcing data integrity.
